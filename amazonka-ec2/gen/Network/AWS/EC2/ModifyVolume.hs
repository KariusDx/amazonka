...
--
-- With previous-generation instance types, resizing an EBS volume may require detaching and reattaching the volume or stopping and restarting the instance. For more information, see <http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ebs-expand-volume.html Modifying the Size, IOPS, or Type of an EBS Volume on Linux> and <http://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/ebs-expand-volume.html Modifying the Size, IOPS, or Type of an EBS Volume on Windows> .
--
...
--
-- * 'mvSize' - The target size of the volume, in GiB. The target volume size must be greater than or equal to than the existing size of the volume. For information about available EBS volume sizes, see <http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EBSVolumeTypes.html Amazon EBS Volume Types> . Default: If no size is specified, the existing size is retained.
--
-- * 'mvIOPS' - The target IOPS rate of the volume. This is only valid for Provisioned IOPS SSD (@io1@ ) volumes. For more information, see <http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EBSVolumeTypes.html#EBSVolumeTypes_piops Provisioned IOPS SSD (io1) Volumes> . Default: If no IOPS value is specified, the existing value is retained.
--
...
-- | The target size of the volume, in GiB. The target volume size must be greater than or equal to than the existing size of the volume. For information about available EBS volume sizes, see <http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EBSVolumeTypes.html Amazon EBS Volume Types> . Default: If no size is specified, the existing size is retained.
mvSize :: Lens' ModifyVolume (Maybe Int)
mvSize = lens _mvSize (\ s a -> s{_mvSize = a})

-- | The target IOPS rate of the volume. This is only valid for Provisioned IOPS SSD (@io1@ ) volumes. For more information, see <http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EBSVolumeTypes.html#EBSVolumeTypes_piops Provisioned IOPS SSD (io1) Volumes> . Default: If no IOPS value is specified, the existing value is retained.
