...
instance FromText CapacityReservationInstancePlatform where
    parser = takeLowerText >>= \case
        ...
        e -> fromTextError $ "Failure parsing CapacityReservationInstancePlatform from value: '" <> e
           <> "'. Accepted values: linux/unix, red hat enterprise linux, suse linux, windows, windows with sql server, windows with sql server enterprise, windows with sql server standard, windows with sql server web"
